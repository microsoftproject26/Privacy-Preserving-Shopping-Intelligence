...
**The last row looked like the one that matters, and it was read too generously.** `R3`
personalizes and `R4` trains on the device; both assume a client's own history changes what
they are shown. At 1.0 the model would give everyone the same answer. At 2.34 it looked like
the model was responding to the session.

**It is not.** A client's decisions span different *categories*, and different categories get
different answers whether or not a session model exists. The diagnostic cannot tell those
apart, so it was replaced with one that can.

### The decisive test: does the model pick what a lookup table picks?

`diagnostics.py`, `output/diagnostics_full.json`, over all 438,185 VALIDATION decisions:

| | model | TRAIN transition table |
|---|---:|---:|
| **accuracy@1** | **0.8241** | **0.8237** |
| top-1 agreement between them | **98.2%** | |
| top-1 equal to its own category's modal prediction | 98.0% | |

**On what to put first, a 2.3-million-parameter GRU and a one-step lookup table are the same
system.** They agree on 98.2% of decisions and their accuracy@1 differs by `0.0004`.

The MRR gain is still real — `0.3479` against `0.3143`, three seeds, spread `0.0007` — but
it now has a precise location: **the entire advantage is in how the model orders ranks 2
through 20.** That is worth something for a five-item carousel and close to nothing for a
single recommendation.

**What this changes:**

* **The product claim narrows.** "We predict what you browse next" is delivered by the
  lookup. What the model adds is a better set of *alternatives*.
* **`R3` and `R4` have less to personalize than assumed.** If the first answer is a category
  lookup, per-client adaptation can only move the tail — and the windows are session-local,
  so there is no cross-session state for it to adapt on either.
* **`S2-DS-05` (architecture) has a clear target.** Any change that does not move accuracy@1
  away from the lookup is refining the tail, and should be reported as that.

Two further diagnostics, both reassuring:

| | measured | reading |
|---|---:|---|
| popularity correlation of the mean score vector | `+0.3059` | moderate — not an expensive popularity table |
| mean \|confidence − accuracy\| over populated bins | `0.0344` | well calibrated; the product **can** threshold on confidence |
