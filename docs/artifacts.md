...
| `FLRuntimeWorkloadConfig` | `config/federated/fl_runtime_workload.v1.json` | v1 | S2-PR-01 | S2-PR-01, S2-PR-03, CI |
| `FLScaleMatrixConfig` | `config/federated/fl_scale_matrix.v1.json` | v1 | S2-PR-01 | S2-PR-01, CI |
| `FLProfileMatrixConfig` | `config/federated/fl_profile_matrix.v1.json` | v1 | S2-PR-03 | S2-PR-03, CI |
| `ConvergencePolicyConfig` | `config/federated/convergence_policy.v1.json` | v1 | S2-PR-02 | S2-PR-02, CI |
| `FLRuntimeBenchmarkCLI` | `scripts/federated/fl_runtime_benchmark.py` | v1 | S2-PR-01 | S2-PR-01, S2-PR-03, CI |
| `FLRuntimeMonitor` | `ppsi/federated/runtime_monitor.py` | v1 | S2-PR-01 | S2-PR-01, S2-PR-03 |
| `ConvergenceModule` | `ppsi/federated/convergence.py` | v1 | S2-PR-02 | #54, #55, Evaluators |
| `ConvergenceValidationCLI` | `scripts/federated/validate_convergence.py` | v1 | S2-PR-02 | CI, Developers |
| `ConvergenceCaseFixture` | `fixtures/federated/convergence_cases.v1.json` | v1 | S2-PR-02 | CI, Tests |
| `FLRuntimeInputs` | `docs/evidence/s2-pr-01/runtime_inputs.v1.json` | v1 | S2-PR-01 | Review / Reproducibility |
| `FLRuntimeInitialization` | `docs/evidence/s2-pr-01/runtime_initialization.v1.json` | v1 | S2-PR-01 | Review / Reproducibility |
| `FLScaleSummary` | `docs/evidence/s2-pr-01/scale_summary.v1.json` | v1 | S2-PR-01 | S2-PR-03 gate, Review |
| `ConvergenceValidationEvidence` | `docs/evidence/s2-pr-02/convergence_validation.v1.json` | v1 | S2-PR-02 | Review / Reproducibility |
| `FLProfileSummary` | `docs/evidence/s2-pr-03/profile_summary.v1.json` | v1 | S2-PR-03 | Review / Reproducibility |
| `FLRuntimeRecommendations` | `docs/evidence/s2-pr-03/runtime_recommendations.v1.json` | v1 | S2-PR-03 | Developers, Review |
| `FLRuntimeExecutionDecision` | `docs/decisions/s2-pr-01-03-execution.md` | v1 | S2-PR-01 | Accepted Decision, Review |
| `FLRuntimeNotebook` | `notebooks/S2_PR_01_03_Federated_Runtime.ipynb` | v1 | S2-PR-01 | Developers, Review |
| `FLScalingDocs` | `docs/federated-scaling.md` | v1 | S2-PR-01 | Developers |
| `FLConvergenceDocs` | `docs/federated-convergence.md` | v1 | S2-PR-02 | Developers |
| `ConvergenceTests` | `tests/federated/test_convergence.py` | v1 | S2-PR-02 | CI |
| `FLRuntimeBenchmarkTests` | `tests/federated/test_runtime_benchmark.py` | v1 | S2-PR-01 | CI |
...
Scoped T1 centralized/FedAvg pilot, run `mvp-t1-003`. Preliminary demonstrator: not the
final R1 denominator, not a full R2A reference, and not a Phase-1 freeze. Private
outputs under `artifacts/mvp/` (prepared memmaps, checkpoints, per-client ledgers,
submission bundle) stay ignored and are never published.

| Logical ID | Canonical path | What it is |
|---|---|---|
| `mvp_execution_policy_v1` | `config/mvp/execution.v3.json` | Resolved policy for the executed scoped T1 pilot |
| `mvp_prepare_summary_v1` | `docs/evidence/mvp/mvp-t1-003/prepare_summary.v1.json` | Counts, digests and raw provenance of the prepared inputs |
| `mvp_preflight_v1` | `docs/evidence/mvp/mvp-t1-003/preflight.v1.json` | Resolved architecture and the common initialization declaration |
| `mvp_source_snapshot_v1` | `docs/evidence/mvp/mvp-t1-003/source_snapshot.v1.json` | Behaviour-defining source frozen before execution |
| `mvp_baseline_summary_v1` | `docs/evidence/mvp/mvp-t1-003/baseline_summary.v1.json` | Three count baselines at full and pilot fit scope |
| `mvp_canary_v1` | `docs/evidence/mvp/mvp-t1-003/canary.v1.json` | Mechanical pre-execution checks; not a scientific result |
| `mvp_centralized_run_v1` | `docs/evidence/mvp/mvp-t1-003/centralized_run.v1.json` | Centralized pilot run record |
| `mvp_federated_run_v1` | `docs/evidence/mvp/mvp-t1-003/federated_run.v1.json` | FedAvg pilot run record with per-round oracle diagnostics |
| `mvp_comparison_v1` | `docs/evidence/mvp/mvp-t1-003/comparison.v1.json` | Matched-exposure comparison of the pair |
| `mvp_pilot_quality_retention_v1` | `docs/evidence/mvp/mvp-t1-003/pilot_quality_retention.v1.json` | Quality retention on the isolated pilot pair only |
| `mvp_verification_v1` | `docs/evidence/mvp/mvp-t1-003/verification.v1.json` | Independent re-derivation of every published claim |
| `mvp_report_inputs_v1` | `docs/evidence/mvp/mvp-t1-003/report_inputs.v1.json` | Verified numbers used by the report, notebook and deck |
| `mvp_pilot_pair_registry` | `docs/evidence/mvp/mvp-t1-003/pilot-pair/` | Isolated ExperimentConfig/Result pair for the pilot |
| `mvp_t1_baseline_results` | `docs/evidence/s2-pr-08/mvp-t1-003/` | Task-local count-baseline results through the frozen evaluator |
| `mvp_federated_attempt_failed_v1` | `docs/evidence/mvp/mvp-t1-001/federated_attempt_failed.v1.json` | Retained failed attempt that motivated the oracle policy |
| `mvp_execution_decision` | `docs/decisions/mvp-execution.md` | Predeclared scope and identity conventions |
| `mvp_readiness` | `docs/mvp-readiness.md` | GO_T1_MVP_ONLY readiness matrix |
| `mvp_results_notebook` | `notebooks/MVP_Results.ipynb` | Evidence-reading demonstration notebook |
